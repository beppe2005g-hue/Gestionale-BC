              ...
              {fattureFiltrate.length === 0 ? (
                ...
                return (
                  <tr key={f.id} className={nc ? 'bg-purple-50' : stato === 'pagata' ? 'opacity-60' : ''}>
                    ...
                    <td className={`text-sm font-medium ${nc ? 'text-purple-700' : ''}`}>{nc ? '- ' : ''}{euro(f.imponibile)}</td>
                    <td className="font-medium text-sm">
                      {nc ? <span className="text-purple-600 text-xs">—</span> : (
                        <>{euro(f.totale)}{stato === 'pagata' && <span className="ml-1 text-xs text-green-600">✓</span>}{stato === 'parziale' && <span className="ml-1 text-xs text-amber-600">½</span>}</>
                      )}
                    </td>
                    {nc ? (
