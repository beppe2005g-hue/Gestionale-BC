...
export async function POST(req: NextRequest) {
  try {
    ...
    // Usa Gemini Flash — costa ~0.001$ per immagine
        ...
        body: JSON.stringify({
          contents: [{
            parts: [
              ...
              {
                text: `Analizza questa bolla DDT italiana. Restituisci SOLO un array JSON valido senza testo prima o dopo:
...
Se la pagina non contiene un DDT rispondi: [{"skip":true}]`
              }
            ]
          }],
          generationConfig: {
    ...
    const filtrati = ddtArray.filter(d => !d.skip && d.numero !== undefined)
