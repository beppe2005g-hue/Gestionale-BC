...
export default function FattureClienti() {
  ...
  async function registraPagamento(rataTarget: number) {
    ...
    if (nuovoPagato >= importoRata - 0.01) {
      await supabase.from('fatture_clienti').update({ [`rata${rataTarget}_stato`]: 'Incassata' }).eq('id', modalDettaglio.id)
    } else {
      await supabase.from('fatture_clienti').update({ [`rata${rataTarget}_stato`]: 'Parziale' }).eq('id', modalDettaglio.id)
    }
  ...
  async function eliminaPagamento(pagamentoId: string, fatturaId: string, rata: number, importoRata: number) {
    ...
    if (pagatoAggiornato >= importoRata - 0.01 && importoRata > 0) nuovoStato = 'Incassata'
    else if (pagatoAggiornato > 0) nuovoStato = 'Parziale'
    await supabase.from('fatture_clienti').update({ [`rata${rata}_stato`]: nuovoStato }).eq('id', fatturaId)
    await load()
    if (modalDettaglio?.id === fatturaId) {
      const { data: aggiornata } = await supabase.from('fatture_clienti').select('*').eq('id', fatturaId).single()
      if (aggiornata) setModalDettaglio(aggiornata)
    }
  }

  async function elimina(id: string, numero: string) {
    if (!confirm(`Eliminare la fattura ${numero}? Verrà eliminato anche lo storico dei pagamenti registrati.`)) return
  ...
  return (
    <div className="flex min-h-screen">
      ...
      {/* ── MODAL DETTAGLIO FATTURA (con cronologia pagamenti) ── */}
      {modalDettaglio && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setModalDettaglio(null)} className="text-gray-400 hover:text-gray-600 text-xl">×</button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4 mb-4 text-sm">
              ...
              <div><span className="text-gray-400 text-xs block">Imponibile</span>{euro(modalDettaglio.imponibile)}</div>
                  ...
                  <div key={n} className="border border-gray-200 rounded-lg p-3">
                        ...
                        {pagamentiRata.map(p => (
                          <div key={p.id} className="flex items-center justify-between text-xs">
                            <span className="text-gray-600">
                              💰 {euro(p.importo)} — {new Date(p.data_pagamento).toLocaleDateString('it-IT')}
                    ...
                    {stato !== 'incassata' && (
                      <div className="flex items-center gap-2 mt-2 pt-2 border-t border-gray-100">
                        ...
                        <button className="btn btn-sm btn-success" disabled={loadingPagamento}
                          onClick={() => registraPagamento(n)}>
