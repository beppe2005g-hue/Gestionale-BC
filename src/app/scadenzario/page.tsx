...
export default function Scadenzario() {
  ...
  async function load() {
    ...
    const [{ data: ff }, { data: fc }] = await Promise.all([
      supabase.from('fatture_fornitori').select('id,numero,fornitore_nome,progetto_nome,rata1_importo,rata1_scadenza,rata1_stato,rata2_importo,rata2_scadenza,rata2_stato,rata3_importo,rata3_scadenza,rata3_stato'),
      supabase.from('fatture_clienti').select('id,numero,cliente_nome,progetto_nome,rata1_importo,rata1_scadenza,rata1_stato,rata2_importo,rata2_scadenza,rata2_stato,rata3_importo,rata3_scadenza,rata3_stato'),
    ])
    ...
    ;(ff || []).forEach(f => {
      ;[1,2,3].forEach(n => {
        ...
        if (imp > 0 && scad) {
          ...
          all.push({ tipo: 'Pagamento', soggetto: f.fornitore_nome, cantiere: f.progetto_nome, fattura: f.numero, rata: n, importo: imp, scadenza: scad, gg, stato })
        }
      })
    })
    ;(fc || []).forEach(f => {
      ;[1,2,3].forEach(n => {
        ...
        if (imp > 0 && scad) {
          const gg = Math.round((new Date(scad).getTime() - Date.now()) / 86400000)
          all.push({ tipo: 'Incasso', soggetto: f.cliente_nome, cantiere: f.progetto_nome, fattura: f.numero, rata: n, importo: imp, scadenza: scad, gg, stato })
        }
      })
    })
      ...
      <main className="flex-1 p-6 overflow-auto">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex gap-2">
            {['tutti','scaduti','questa_settimana','pagamenti','incassi','chiusi'].map(f => (
              <button key={f} onClick={() => setFiltro(f)}
                className={`btn btn-sm ${filtro === f ? 'btn-primary' : ''}`}>
                {f === 'tutti' ? 'Tutti' : f === 'scaduti' ? '🔴 Scaduti' : f === 'questa_settimana' ? '🟡 7 gg' : f === 'pagamenti' ? 'Pagamenti' : f === 'incassi' ? 'Incassi' : '✅ Chiusi'}
              </button>
            ))}
          </div>
        </div>
